...
class Ranking(object):
    ...
    def ranking():
        ...
        # ログデータを取得
        query2 = client.query(kind='Sensorlog')
        logs = list(query2.fetch())

        # 配艇情報を取得
        query3 = client.query(kind='Outline_yacht_player')
        query3.add_filter('outline_id', '=', int(target_outline_id))
        haitei = list(query3.fetch())
        print(haitei)
